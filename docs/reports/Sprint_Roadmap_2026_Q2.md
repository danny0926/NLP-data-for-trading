...
### 本週 (W1)
- [ ] 設定 Social Media API credentials (APIFY_API_TOKEN, REDDIT_*) — **Pillar B 最高優先**
- [x] 首次執行 `python -m src.signal_tracker` 開始追蹤信號績效 — 39 signals, +2.50% CAR_5d, 64.1% HR
- [ ] 設定 WSL2 cron 排程 (`bash cron_setup.sh`)
- [x] FastAPI v2.0 — rate limiter, 15 endpoints, OpenAPI tags (2026-03-05)
- [ ] 設定 TELEGRAM_BOT_TOKEN — 啟用推送告警
- [ ] JWT 認證 (deferred — API Key 已夠用)

### 本月 (March)
- [ ] **RB-009 Phase 3**: 歷史合約回填 + BUY+contract vs BUY-only alpha 回測
- [ ] **RB-009 自動化**: USASpending daily fetch 加入 run_daily.py
- [x] Signal Performance Dashboard (Streamlit 新頁面) — **2026-03-05 完成**
- [x] Sector Rotation Dashboard (Streamlit 新頁面) — **2026-03-05 完成**
- [ ] Docker 容器化
- [ ] API 產品化文檔 (OpenAPI spec)
- [ ] TD-002: Senate ETL 健康告警 (Telegram 通知)
- [x] TD-005/008: bare except 清理 (src/ only 2 left in legacy) + Gemini timeout 120s — **2026-03-05**
...
## 8. Autopilot Sprint (2026-03-05)

### Data Refresh
| Item | Before | After | Change |
|------|--------|-------|--------|
| congress_trades | 404 | 484 | +80 (House +32, Senate +48) |
| sec_form4_trades | 336 | 696 | +360 |
| signal_quality_scores | 404 | 484 | +80 |
| alpha_signals | 355 | 409 | +54 |
| enhanced_signals | 355 | 409 | +54 |
| signal_performance | 48 | 60 | +12 |

### Code Improvements
| Item | Details | Commit |
|------|---------|--------|
| Gemini API timeout | 120s on all 3 clients | 0c27373 |
| FastAPI v2.0 | Rate limiter, 4 new endpoints (15 total), OpenAPI tags | 0c27373 |
| Bare except cleanup | 3 fixes in src/ (sector_radar, analyze_optimal_timing) | 0c27373 |
| Streamlit 9 pages | +Signal Performance, +Sector Rotation | 8486cae |

### Top Signals (2026-03-05)
| Ticker | Direction | PACS | Enhanced Strength | Politician | Convergence |
|--------|-----------|------|-------------------|------------|-------------|
| ETN | LONG | 0.776 | 0.388 | Gilbert Cisneros + John Boozman | YES (cross-chamber) |
| AAPL | LONG | 0.554 | 0.277 | Nancy Pelosi + John Boozman | YES (cross-chamber) |
| STT | LONG | 0.580 | 0.290 | Jake Auchincloss | No |
| COIN | LONG | 0.587 | 0.293 | Gilbert Cisneros | No |

### Blocking Issues
- **APIFY_API_TOKEN** not set — Pillar B (social media) completely empty
- **TELEGRAM_BOT_TOKEN** not set — 40 smart alerts generated but can't push
- **REDDIT_* credentials** not set — Reddit scraping blocked

*Generated by PAM Autopilot | 2026-03-05*
